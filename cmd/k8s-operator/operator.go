...
func runReconcilers(opts reconcilerOpts) {
	...
	err = builder.
		ControllerManagedBy(mgr).
		Named("egress-readiness-reconciler").
		Watches(&discoveryv1.EndpointSlice{}, podsForEps).
		Watches(&corev1.Pod{}, podsER).
		Complete(&egressReadinessReconciler{
			Client:      mgr.GetClient(),
			tsNamespace: opts.tailscaleNamespace,
			clock:       tstime.DefaultClock{},
			logger:      opts.log.Named("egress-readiness-reconciler"),
		})
	if err != nil {
...
func egressPodsHandler(_ context.Context, o client.Object) []reconcile.Request {
	...
	// TODO: get proxygroup, check that type is egress
	return []reconcile.Request{
		{
			NamespacedName: types.NamespacedName{
				Namespace: o.GetNamespace(),
...
func podsFromEgressEps(cl client.Client, logger *zap.SugaredLogger, ns string) handler.MapFunc {
	return func(ctx context.Context, o client.Object) []reconcile.Request {
		...
		log.Printf("TEST: running with endpointslice %v", eps.Name)
		// TODO: ensure these have service type label
